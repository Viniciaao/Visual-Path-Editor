...
t.describe('render - projecao e culling', function()
	...
	t.test('desenha cada link uma unica vez', function()
		mock.reset()
		local p, r = fixture({ render = { ativo = true, distancia = 250, links_modo = 'todos' } })
		r:update()
	...
	t.test('respeita o maximo de linhas por quadro', function()
		...
		p:loadArea(16, area)
		local r = render.new(p, { render = { ativo = true, distancia = 250, max_linhas = 10, tamanho_node = 4, links_modo = 'todos' } })
		r:update()
		t.eq(r:drawLinks(), 10)
	...
	t.test('navi nodes sao desenhados com a direcao', function()
		...
		r:update()
		t.eq(r:drawNavis(), 1)
		t.eq(#callsOf('box'), 0, 'navi node nao e mais quadrado cheio')
		local lines = callsOf('line')
		t.eq(#lines, 3, 'cruz (2 tracos) + tracinho da direcao')
	end)
	...
	t.test('poligonos so quando pedido na configuracao', function()
		mock.reset()
		local p, r = fixture({ render = { ativo = true, distancia = 250, usar_poligonos = true, escala_por_distancia = false, tamanho_node = 6 } })
		r:update()
		t.eq(r:drawNodes(), 3)
		t.eq(#callsOf('poly'), 3)
		t.eq(#callsOf('box'), 0)
	...
	t.test('tamanho do node acompanha a distancia (parece estar no mundo)', function()
		...
		r4:update()
		r4:drawNodes()
		t.near(callsOf('box')[1][4], 12.0, 0.001, 'raio maximo 6 px (marcador discreto)')
		mock.reset()
	end)
	...
	t.test('links: padrao desenha so as ligacoes do node selecionado', function()
		mock.reset()
		local p, r = fixture({ render = { ativo = true, distancia = 250, links_modo = 'selecionado' } })
		r:update()
		t.eq(r:drawLinks(), 0, 'nada selecionado: nenhuma linha')

		mock.reset()
		p:select(15, 1)
		r:update()
		t.eq(r:drawLinks(), 1, 'so o link do node escolhido')

		mock.reset()
		p:select(15, 3) -- pedestre, sem links
		r:update()
		t.eq(r:drawLinks(), 0)

		-- modo 'todos': a rede inteira
		mock.reset()
		local p2, r2 = fixture({ render = { ativo = true, distancia = 250, links_modo = 'todos' } })
		r2:update()
		t.eq(r2:drawLinks(), 1)

		-- modo 'nenhum'
		mock.reset()
		local p3, r3 = fixture({ render = { ativo = true, distancia = 250, links_modo = 'nenhum' } })
		r3:update()
		t.eq(r3:drawLinks(), 0)
	end)

	t.test('link do vizinho do node selecionado tambem aparece', function()
		mock.reset()
		local p, r = fixture({ render = { ativo = true, distancia = 250, links_modo = 'selecionado' } })
		p:select(15, 2) -- o node 2 tem o link de volta para o node 1
		r:update()
		t.eq(r:drawLinks(), 1, 'o link aparece pelo lado do vizinho')
	end)

	t.test('navi node so e desenhado dentro do alcance dos navis', function()
		mock.reset()
		local p, r = fixture({ render = { ativo = true, distancia = 500, distancia_navis = 60, mostrar_navis = true } })
		p:addNavi(15, 2497, -1684, 15, 1)
		r:update()
		t.eq(r:drawNavis(), 1, 'perto: desenha')

		mock.reset()
		local p2, r2 = fixture({ render = { ativo = true, distancia = 500, distancia_navis = 60, mostrar_navis = true } })
		local navi = p2:addNavi(15, 2497, -1684, 15, 1)
		local area = p2:area(15)
		area.navis[navi].x = 2495 + 200 -- 200 m de distancia
		r2:update()
		t.eq(r2:drawNavis(), 0, 'longe: nao desenha')
	end)

	t.test('node selecionado ganha cruz de destaque', function()
		mock.reset()
		local p, r = fixture({ render = { ativo = true, distancia = 250, escala_por_distancia = false, tamanho_node = 6 } })
		r:update()
		r:drawNodes()
		local semSelecao = #callsOf('line')

		mock.reset()
		p:select(15, 1)
		r:update()
		r:drawNodes()
		local comSelecao = #callsOf('line')
		t.ok(comSelecao > semSelecao or semSelecao == 2, 'cruz do node escolhido desenhada')
		t.eq(comSelecao, 2, 'cruz = 2 tracos')
	end)
